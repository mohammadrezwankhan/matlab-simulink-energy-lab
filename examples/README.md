...
| [Switching closed-loop buck](converter-switching-closed-loop-model/README.md) | Connects bounded period-sampled control to an event-aligned PWM plant with diode, copper, source-backed nominal SiC switch losses, and fixed-junction-temperature sensitivity. | `converter-switching-closed-loop-model/simulate_switching_closed_loop_buck.m`, `converter-switching-closed-loop-model/evaluate_switching_closed_loop_buck_temperature_sensitivity.m` | `run_switching_closed_loop_buck`, `check_switching_closed_loop_buck`, `run_switching_closed_loop_buck_temperature_sensitivity`, `check_switching_closed_loop_buck_temperature_sensitivity` |
...
| `converter-switching-closed-loop-model/check_switching_closed_loop_buck.m` | Verifies period-sampled regulation, limits, event counts, semiconductor-loss separation, averaged and energy balances, zero-loss parity, grid/frequency sensitivity, and invalid inputs. |
...
| `converter-switching-closed-loop-model/check_switching_closed_loop_buck_temperature_sensitivity.m` | Verifies exact 25 degC baseline parity, datasheet anchors, monotonic fixed-temperature loss sensitivity, energy closure, determinism, doubled-grid convergence, and range rejection. |
| `converter-switching-closed-loop-model/run_switching_closed_loop_buck_temperature_sensitivity.m` | Plots the sourced temperature interpolation, separated switch loss, estimated efficiency, and regulated final output. |
| `converter-switching-closed-loop-simulink-model/check_switching_closed_loop_buck_simulink_model.m` | Generates the native diagram twice, checks topology, and verifies exact state, PWM, duty, current-reference, and event parity with Base MATLAB. |
| `converter-switching-closed-loop-simulink-model/run_switching_closed_loop_buck_simulink_model.m` | Generates and opens the native diagram and overlays its electrical states with the Base MATLAB reference. |
