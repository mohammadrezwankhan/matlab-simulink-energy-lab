function estimate = estimate_battery_soc_hysteresis_ekf( ...
        time_s, current_A, measuredVoltage_V, parameters, options)
%ESTIMATE_BATTERY_SOC_HYSTERESIS_EKF Estimate SOC, RC voltage, and hysteresis.
% Positive current denotes discharge. The state is [SOC; Vrc; h].

[time_s, current_A, measuredVoltage_V, parameters, options] = ...
    validate_inputs(time_s, current_A, measuredVoltage_V, parameters, options);

sampleCount = numel(time_s);
state = zeros(3, sampleCount);
priorState = zeros(3, sampleCount);
covariance = zeros(3, 3, sampleCount);
priorCovariance = zeros(3, 3, sampleCount);
...
kalmanGain = zeros(3, sampleCount);
...
for sampleIndex = 1:sampleCount
    ...
    measurementJacobian = [ ...
        ocvSlope_V, -1, parameters.hysteresis_max_V];
    priorTerminalVoltage_V = priorOcv_V + ...
        parameters.hysteresis_max_V*xPrior(3) - ...
        parameters.r0_Ohm*current_A(sampleIndex) - xPrior(2);
    innovation = measuredVoltage_V(sampleIndex) - priorTerminalVoltage_V;
    ...
    xPosterior(3) = min(max(xPosterior(3), -1), 1);
    ...
    normalizedInnovationSquared(sampleIndex) = innovation^2/innovationVariance;
    kalmanGain(:, sampleIndex) = gain;
    [posteriorOcv_V, ~] = ocv_with_slope( ...
        xPosterior(1), parameters.ocv_soc_breakpoints, ...
        parameters.ocv_lookup_V);
    estimatedTerminalVoltage_V(sampleIndex) = posteriorOcv_V + ...
        parameters.hysteresis_max_V*xPosterior(3) - ...
        parameters.r0_Ohm*current_A(sampleIndex) - xPosterior(2);

    if sampleIndex < sampleCount
        interval_s = time_s(sampleIndex + 1) - time_s(sampleIndex);
        rcDecay = exp(-interval_s/(parameters.r1_Ohm*parameters.c1_F));
        if current_A(sampleIndex) == 0 || ...
                parameters.hysteresis_rate_per_fractional_throughput == 0
            hysteresisDecay = 1;
            nextHysteresis = xPosterior(3);
        else
            fractionalThroughput = abs(current_A(sampleIndex))*interval_s/ ...
                (3600*parameters.capacity_Ah);
            hysteresisDecay = exp( ...
                -parameters.hysteresis_rate_per_fractional_throughput* ...
                fractionalThroughput);
            target = -sign(current_A(sampleIndex));
            nextHysteresis = target + ...
                (xPosterior(3) - target)*hysteresisDecay;
        end
        stateTransition = diag([1, rcDecay, hysteresisDecay]);
        xPrior = [
            xPosterior(1) - current_A(sampleIndex)*interval_s/ ...
                (3600*parameters.capacity_Ah);
            rcDecay*xPosterior(2) + parameters.r1_Ohm*(1 - rcDecay)* ...
                current_A(sampleIndex);
            nextHysteresis
        ];
        xPrior(1) = min(max(xPrior(1), 0), 1);
        xPrior(3) = min(max(xPrior(3), -1), 1);
        processCovariance = ...
...
estimate.hysteresis_state = state(3, :)';
estimate.prior_soc = priorState(1, :)';
estimate.prior_v_rc_V = priorState(2, :)';
estimate.prior_hysteresis_state = priorState(3, :)';
...
function [time_s, current_A, voltage_V, parameters, options] = ...
        validate_inputs(time_s, current_A, voltage_V, parameters, options)
signals = {time_s, current_A, voltage_V};
for signalIndex = 1:numel(signals)
    value = signals{signalIndex};
    if ~isnumeric(value) || ~isvector(value) || ~isreal(value) || ...
            any(~isfinite(value))
        error('BatterySOCHysteresisEKF:Signals', ...
            'Signals must be finite real numeric vectors.');
    end
end
time_s = time_s(:);
current_A = current_A(:);
...
if numel(time_s) < 2 || numel(current_A) ~= numel(time_s) || ...
        numel(voltage_V) ~= numel(time_s)
    error('BatterySOCHysteresisEKF:SignalShape', ...
        'Signals must have equal length of at least two.');
end
if any(diff(time_s) <= 0)
    error('BatterySOCHysteresisEKF:TimeOrder', ...
        'Time samples must increase strictly.');
end

requiredScalars = {'capacity_Ah'; 'r0_Ohm'; 'r1_Ohm'; 'c1_F'; ...
    'hysteresis_max_V'; 'hysteresis_rate_per_fractional_throughput'};
requiredParameters = [requiredScalars; ...
    {'ocv_soc_breakpoints'; 'ocv_lookup_V'}];
if ~isstruct(parameters) || ~all(isfield(parameters, requiredParameters))
    error('BatterySOCHysteresisEKF:Parameters', ...
        'The parameter structure is missing a required field.');
end
for index = 1:numel(requiredScalars)
    value = parameters.(requiredScalars{index});
...
if parameters.capacity_Ah <= 0 || parameters.r0_Ohm < 0 || ...
        ...
        parameters.hysteresis_rate_per_fractional_throughput < 0
    error('BatterySOCHysteresisEKF:Parameters', ...
        'Electrical parameters are outside their physical bounds.');
end
breakpoints = parameters.ocv_soc_breakpoints(:);
lookup = parameters.ocv_lookup_V(:);
if ~isnumeric(breakpoints) || ~isnumeric(lookup) || ...
        numel(breakpoints) < 2 || numel(breakpoints) ~= numel(lookup) || ...
        any(~isfinite(breakpoints)) || any(~isfinite(lookup)) || ...
        abs(breakpoints(1)) > 1e-12 || abs(breakpoints(end) - 1) > 1e-12 || ...
        any(diff(breakpoints) <= 0) || any(lookup <= 0) || ...
        any(diff(lookup) < 0)
    error('BatterySOCHysteresisEKF:OCVCurve', ...
        'OCV data must be positive and nondecreasing on SOC [0, 1].');
end
parameters.ocv_soc_breakpoints = breakpoints;
parameters.ocv_lookup_V = lookup;

requiredOptions = {'initial_state'; 'initial_covariance'; ...
    'process_noise_covariance_per_s'; 'measurement_variance_V2'};
if ~isstruct(options) || ~all(isfield(options, requiredOptions)) || ...
        ~isnumeric(options.initial_state) || ...
        ~isequal(size(options.initial_state), [3, 1]) || ...
        ~isreal(options.initial_state) || ...
        any(~isfinite(options.initial_state)) || ...
        options.initial_state(1) < 0 || options.initial_state(1) > 1 || ...
        abs(options.initial_state(3)) > 1
    error('BatterySOCHysteresisEKF:Options', ...
        'Options require a bounded finite [SOC; Vrc; h] initial state.');
end
...
if ~isnumeric(options.measurement_variance_V2) || ...
    ...
    error('BatterySOCHysteresisEKF:MeasurementVariance', ...
        'Measurement variance must be a finite positive scalar.');
end
end
...
if ~isnumeric(covariance) || ~isequal(size(covariance), [3, 3]) || ...
    ...
    error('BatterySOCHysteresisEKF:Covariance', ...
        '%s must be a symmetric positive-semidefinite 3-by-3 matrix.', ...
        fieldName);
end
end
