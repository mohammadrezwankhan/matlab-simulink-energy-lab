...
nexttile;
plot(result.interval_start_time_s, cumsum( ...
    result.switch_conduction_loss_power_W*result.time_step_s), ...
    'LineWidth', 1.1);
hold on;
stairs(result.interval_start_time_s, cumsum( ...
    result.switching_loss_power_W*result.time_step_s), ...
    'LineWidth', 1.1);
plot(result.interval_start_time_s, cumsum( ...
    result.diode_loss_power_W*result.time_step_s), ...
    'LineWidth', 1.1);
grid on;
xlabel('Time [s]');
ylabel('Cumulative loss energy [J]');
legend('Switch conduction', 'Turn-on/off', 'Freewheel diode', ...
    'Location', 'best');
title('Separated Semiconductor Losses');
