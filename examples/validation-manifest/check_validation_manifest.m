...
[checks, requiresSimulink] = validation_check_catalog(false);
[allChecks, allRequireSimulink] = validation_check_catalog(true);
[baseChecks, baseRequiresSimulink] = ...
    validation_check_catalog(false, false);
baseChecksWithBessRequested = validation_check_catalog(true, false);
...
assert(numel(baseChecks) == 20 && ...
    isequal(baseChecks, checks(~requiresSimulink)) && ...
    isequal(baseChecksWithBessRequested, baseChecks) && ...
    nnz(requiresSimulink) == 5 && ...
    nnz(allRequireSimulink) == 6 && allRequireSimulink(end) && ...
    ~any(baseRequiresSimulink) && ...
    ~any(contains(baseChecks, "simulink")), ...
    'The Base MATLAB profile must select exactly 20 ordered checks.');

invalidDependencyFlagRejected = false;
try
    validation_check_catalog(false, 2);
catch validationError
    invalidDependencyFlagRejected = strcmp(validationError.identifier, ...
        'ValidationCatalog:IncludeSimulink');
end
assert(invalidDependencyFlagRejected, ...
    'The catalog must reject a nonlogical dependency flag.');
