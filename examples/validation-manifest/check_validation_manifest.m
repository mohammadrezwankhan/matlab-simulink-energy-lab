...
failedReport.checks(1).error_identifier = "Example:Failure";
failedReport.checks(1).error_message = "Deliberate fixture failure";
failedReport.all_checks_passed = false;
failedManifest = build_validation_manifest(failedReport, sourceCommit);
assert(failedManifest.status == "failed" && ...
    failedManifest.checks(1).status == "failed" && ...
    failedManifest.checks(1).error_identifier == "Example:Failure", ...
    'Failed checks must retain machine-readable status and error evidence.');
