...
for intervalIndex = 1:numel(interval_s)
    ...
    net_heat_W(intervalIndex) = heat_generation_W(intervalIndex) - ...
...
[ocv_V(end), terminal_voltage_V(end), heat_generation_W(end), ...
    cooling_power_W(end)] = calculate_outputs(...
        soc(end), v_rc_V(end), cell_temp_C(end), r0_Ohm(end), ...
        current_A(end), parameters);
net_heat_W(end) = heat_generation_W(end) - cooling_power_W(end);

allSignals = [soc, v_rc_V, ocv_V, terminal_voltage_V, cell_temp_C, ...
    r0_Ohm, heat_generation_W, cooling_power_W, net_heat_W];
...
function [ocv_V, terminalVoltage_V, heatGeneration_W, coolingPower_W] = ...
...
heatGeneration_W = current_A * (ohmicDrop_V + vRc_V);
